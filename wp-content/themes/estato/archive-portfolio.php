...
if ( get_option( 'page_for_posts' ) ) {
	EstatoTheme::$boldthemes_page_for_header_id = get_option( 'page_for_posts' );
}
...
if ( have_posts() ) {
	...
	if ( is_search() ) { ?>
		<article class="btNoSearchResults">
			<?php echo boldthemes_get_heading_html( esc_html__( 'We are sorry, no results for: ', 'estato' ) . get_query_var( 's' ), '', "<a href='" . site_url() . "'>" . esc_html__( 'Back to homepage', 'estato' )."</a>", 'extralarge', 'bottom', '', '' ) ?>
		</article>
	<?php }
}
